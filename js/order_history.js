    ...
    async function handleEditOrder(event) {
        ...
        try {
            ...
            const deliveryTime = order.delivery_time 
                ? order.delivery_time.trim().slice(0, 5) : '';

            console.log("Delivery time before rendering:", order.delivery_time);
            console.log("Formatted delivery time:", deliveryTime);
        ...
        : 'Неизвестно'}" disabled>
                    ...
                    Как можно скорее
                        <input type="radio" name="delivery_type" 
                            value="now" ${order.delivery_type === 'now' 
        ? 'checked' : ''}> 
                    </label>
                    <label>
                    Ко времени
                        <input type="radio" name="delivery_type" 
                            value="by_time" ${order.delivery_type === 'by_time' 
        ? 'checked' : ''}> 
        ...
        `;
            ...
            console.log("Modal rendered with delivery_time:", deliveryTime);
